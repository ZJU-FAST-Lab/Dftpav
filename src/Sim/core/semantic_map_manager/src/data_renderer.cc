...
ErrorType DataRenderer::Render(const double &time_stamp,
  ...
  GetObstacleMap(obstacle_set);  // update obstacles map obstacle_grids_
  ...
  p_semantic_map_manager_->set_static_obs_list(static_obs_list);
  //just to set
  //p_obstacle_grid = cv::mat = gridmap
  //obs_grids = global_point 
...
ErrorType DataRenderer::GetEgoVehicle(const common::VehicleSet &vehicle_set) {
  ...
  // std::cout<<"--------------------------------------------------\n";
  // ego_param_.print();
  // std::cout<<"ego_id: "<<ego_id_<<"\n";
  // std::cout<<"ego_vehicle_id: "<<ego_vehicle_.id()<<"\n";
...
ErrorType DataRenderer::GetObstacleMap(
  ...
  for (const auto &obs : obstacle_set.obs_circle) {
    ...
    //hzc
    std::vector<geometry_msgs::Point> circle_obs;
    geometry_msgs::Point vec1;
    vec1.x = obs.second.circle.center.x+obs.second.circle.radius;
    vec1.y = obs.second.circle.center.y;

    geometry_msgs::Point vec2;
    vec2.x = obs.second.circle.center.x;
    vec2.y = obs.second.circle.center.y-obs.second.circle.radius;
    geometry_msgs::Point vec3;
    vec3.x = obs.second.circle.center.x-obs.second.circle.radius;
    vec3.y = obs.second.circle.center.y;
    geometry_msgs::Point vec4;
    vec4.x = obs.second.circle.center.x;
    vec4.y = obs.second.circle.center.y+obs.second.circle.radius;
    geometry_msgs::Point vec5;
    vec5.x = obs.second.circle.center.x+obs.second.circle.radius;
    vec5.y = obs.second.circle.center.y;
    circle_obs.push_back(vec1);
    circle_obs.push_back(vec2);
    circle_obs.push_back(vec3);
    circle_obs.push_back(vec4);
    circle_obs.push_back(vec5);
    static_obs_list.push_back(circle_obs);
  }
  ...
  for (const auto &obs : obstacle_set.obs_polygon) {
    ...
    //hzchzc
    std::vector<geometry_msgs::Point> poly_obs;
    // std::cout<<"-------------------\n";
    for (const auto &pt : obs.second.polygon.points) {
      ...
      p_obs.x = pt.x;
      p_obs.y = pt.y;
      poly_obs.push_back(p_obs);
      // std::cout<<pt_w[0]<<"  "<<pt_w[1]<<"\n";
    }
