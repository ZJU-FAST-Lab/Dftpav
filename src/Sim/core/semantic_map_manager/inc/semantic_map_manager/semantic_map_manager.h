...
class SemanticMapManager {
 public:
  ...
  /*hzc for obca*/
  inline std::vector<std::vector<geometry_msgs::Point>> get_static_obs_list() const{
    return static_obs_list;
  }
  inline void set_static_obs_list(const std::vector<std::vector<geometry_msgs::Point>>& in){
    static_obs_list = in;
  }
 ...
 private:
  ...
  // * hzc moving obstacle, only used in parking scene
  std::vector<std::vector<common::State>> surpoints;
  ...
  //*for obca planner only 

  std::vector<std::vector<geometry_msgs::Point>> static_obs_list;
