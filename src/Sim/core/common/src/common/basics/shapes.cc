...
ErrorType ShapeUtils::GetDenseVerticesOfOrientedBoundingBox(
      const OrientedBoundingBox2D& obb, vec_E<Vecf<2>>* vertices, double res){
  vertices->clear();
  // vertices->reserve(4*(res));
  decimal_t cos_theta = cos(obb.angle);
  decimal_t sin_theta = sin(obb.angle);
  Vecf<2> corner1(
      obb.x + 0.5 * obb.length * cos_theta + 0.5 * obb.width * sin_theta,
  ...
  for(double dl = res;dl < (corner2-corner1).norm(); dl+=res){
    Vecf<2> point12 = dl / (corner2-corner1).norm() * (corner2-corner1) + corner1;
    vertices->push_back(point12);
  }
  for(double dl = res;dl < (corner3-corner2).norm(); dl+=res){
    Vecf<2> point23 = dl / (corner3-corner2).norm() * (corner3-corner2) + corner2;
    vertices->push_back(point23);
  }
  for(double dl = res;dl < (corner4-corner3).norm(); dl+=res){
    Vecf<2> point34 = dl / (corner4-corner3).norm() * (corner4-corner3) + corner3;
    vertices->push_back(point34);
  }
  for(double dl = res;dl < (corner1-corner4).norm(); dl+=res){
    Vecf<2> point41 = dl / (corner1-corner4).norm() * (corner1-corner4) + corner4;
