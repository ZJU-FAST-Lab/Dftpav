...
class ShapeUtils {
 public:
  ...
  /**
   * @brief Get the clutter Vertices Of OrientedBoundingBox object
   *
   * @param obb Input OBB
   * @param vertices Output clutter vertice vector
   * @return ErrorType
   */
  static ErrorType GetDenseVerticesOfOrientedBoundingBox(
      const OrientedBoundingBox2D& obb, vec_E<Vecf<2>>* vertices,double res = 0.1);
